  ...
  Future<void> register(BuildContext context, String email, String password) async {
    await _auth
      ...
      (value) {
        ...
        SnackBarComponent.successfullMessage(context, 'User created successfully');
      },
    ).onError(
      (error, stackTrace) {
        SnackBarComponent.errorMessage(context,error.toString().replaceAll(RegExp('\\[.*?\\]'), ''),);
      },
    );
  }
  ...
  Future<void> login(BuildContext context, String email, String password) async {
    await _auth
      ...
      (value) {
        emailController.clear();
        passwordController.clear();
        SnackBarComponent.successfullMessage(context, 'User logged in successfully');
      },
    ).onError(
      (error, stackTrace) {
        SnackBarComponent.errorMessage(context,error.toString().replaceAll(RegExp('\\[.*?\\]'), ''),);
  ...
  Future<void> logout(BuildContext context) async {
    await _auth.signOut().then(
      (value) {
        emailController.clear();
        passwordController.clear();
        SnackBarComponent.successfullMessage(context, 'User logged out successfully');
      },
    ).onError(
      (error, stackTrace) {
        SnackBarComponent.errorMessage(context, error.toString().replaceAll(RegExp('\\[.*?\\]'), ''));
